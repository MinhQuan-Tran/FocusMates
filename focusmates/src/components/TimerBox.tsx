...
export default function Timer({ timeLeft, isFocus, totalDuration, small = false }: TimerBoxProps & { small?: boolean }) {
  ...
  return (
    <div className={`${small ? "w-24" : "bg-white shadow-xl p-3 max-w-sm w-full rounded-xl"}`}>
      ...
      >
        <svg
          className={`absolute ${small ? "w-16 h-16" : "w-40 h-40"} rotate-[-90deg]`}
          viewBox="0 0 100 100"
        >
          <circle
            cx="50"
        ...
        <div className={`${small ? "text-md" : "text-4xl"} font-bold text-gray-800`}>
          {formatTime(timeLeft)}
        </div>
      </div>
      ...
      {!small && (
        <Link href="/" className="w-full">
          <button
            className="w-full bg-softYellow text-dark hover:bg-softYellowHover py-2 rounded-lg font-medium transition-colors duration-300"
          >
            Leave Session
          </button>
        </Link>
